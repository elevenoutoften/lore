...
def main() -> int:
    ...
    # 1. capture -------------------------------------------------------------
    text = (f"{MARK}: Nyx agentic smoke test ran against Lore on this date; "
    ...
    record("2. consolidate", status == 200,
    ...
    record("3. recall (default scope)", found_default > 0,
           f"count={found_default}"
           + ("" if found_default else "  <-- captured memory NOT visible under default scope"))
    ...
    if found_default == 0 and found_x > 0:
        record("4. cross_actor diagnostic", True,
        ...
        record("4. cross_actor diagnostic", found_x > 0,
               f"cross_actor count={found_x} actors={actors}")
    ...
    record("6b. rag retrieve-expanded", status == 200,
    ...
    try:
        legacy = client.create_capture(
            ...
            source="testing/nyx-agentic-smoke.py", confidence="medium")
        record("7. legacy capture (Hermes plugin path)",
