...
Page ID format (manual convention): `notes/<agent-slug>/YYYY-MM-DD/daily`

The `notes/<agent-slug>/YYYY-MM-DD/daily` form is a naming convention for a
`kind: capture` page. Use it for end-of-session summaries of what was learned or
done.

In addition, Lore has a built-in session-to-daily distillation feature that
consolidates a day's captures into a canonical daily note at
`dailies/YYYY-MM-DD` with `kind: daily-note`. These pages are generated by the
distillation workflow (`POST /api/distill/daily` or the `lore_distill_daily`
MCP tool), not hand-authored from this template. See
[distillation.md](distillation.md) for the full workflow.

Example (manual convention):
